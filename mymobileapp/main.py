    ...
    try:
        ...
        def show_exam_setup():
            ...
            for t in ["单选题", "多选题", "填空题", "判断题"]:
                max_q = len(db['all'][t])
                row = ft.Row([ft.Text(f"{t}(余{max_q})", width=90), ft.TextField(label="数量", value=str(min(10, max_q)), width=60, keyboard_type=ft.KeyboardType.NUMBER), ft.TextField(label="分值", value="2", width=60, keyboard_type=ft.KeyboardType.NUMBER)], alignment=ft.MainAxisAlignment.SPACE_BETWEEN)
            ...
            def start_exam(e):
                ...
                for t, row in inputs.items():
                    cnt, score = int(row.controls[1].value), float(row.controls[2].value)
                    configs[t] = {'score': score}
                    paper.extend(random.sample(db['all'][t], cnt))
                if not paper: return
                exam_state['paper'], exam_state['config'], exam_state['answers'] = paper, configs, {q['id']: [] if q['type'] == '多选题' else "" for q in paper}
                show_exam_running()
        ...
        def show_exam_running():
            ...
            for idx, q in enumerate(paper):
                ...
                q_col = ft.Column([ft.Text(f"第 {idx+1} 题 ({q['type']} - {score}分)", weight=ft.FontWeight.BOLD), ft.Text(q['question'], size=16)])
                ...
                if q['type'] in ['单选题', '判断题']: q_col.controls.append(ft.RadioGroup(content=ft.Column([ft.Radio(value=opt, label=opt) for opt in (q['options'] if q['options'] else ["对", "错"])]), on_change=record_ans))
                elif q['type'] == '多选题': 
                    for opt in q['options']: q_col.controls.append(ft.Checkbox(label=opt, on_change=lambda e, o=opt: record_multi_ans(e, opt=o)))
                elif q['type'] == '填空题': q_col.controls.append(ft.TextField(hint_text="输入答案", on_change=record_ans))
            ...
            for q in paper:
                ...
                if q['type'] in ['单选题', '多选题', '判断题']:
                    ...
                    if std_match:
                        ...
                        user_extracted = ""
                        if user_ans:
                            if q['type'] == '单选题': m = re.match(r'^([A-Z])[\.．、]', user_ans); user_extracted = m.group(1) if m else ""
                            elif q['type'] == '多选题': letters = [re.match(r'^([A-Z])[\.．、]', opt).group(1) for opt in user_ans if re.match(r'^([A-Z])[\.．、]', opt)]; user_extracted = "".join(sorted(letters))
