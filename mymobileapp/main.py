...
def main(page: ft.Page):
    ...
    try:
        ...
        # --- 1. 安全读取题库 (兼容安卓与电脑双端路径) ---
        def load_built_in_banks():
            # 强制指向标准 assets 目录
            assets_dir = "assets"
            
            if not os.path.exists(assets_dir):
                raise Exception(f"未找到 assets 文件夹！当前运行目录: {os.getcwd()}")
            
            files = [f for f in os.listdir(assets_dir) if f.endswith(".docx")]
            if not files:
                raise Exception(f"assets 文件夹内没有找到任何 .docx 题库文件！")
        ...
        def show_exam_setup():
            ...
            for t in ["单选题", "多选题", "填空题", "判断题"]:
                max_q = len(db['all'][t])
                row = ft.Row([
                    ft.Text(f"{t}(余{max_q})", width=90),
                    ft.TextField(label="数量", value=str(min(10, max_q)), width=60, keyboard_type=ft.KeyboardType.NUMBER),
        ...
        def show_exam_running():
            ...
            for idx, q in enumerate(paper):
                ...
                if q['type'] in ['单选题', '判断题']:
                    ...
                    q_col.controls.append(ft.TextField(hint_text="输入答案", on_change=record_ans))
                ...
                ft.Card(
                    ...
                    content=ft.Container(
                        ...
                        content=ft.Column([
                            ...
                            ft.Text(f"客观题得分: {obj_score:g} / {total_obj:g}", size=18, color=ft.colors.WHITE),
                            ft.Text(f"主观题满分: {total_sub:g}", size=18, color=ft.colors.WHITE70),
                        ])
                    )
                )
            )
        ...
        page.controls.clear()
        page.add(
            ft.AppBar(title=ft.Text("⚠️ 启动保护模式", color=ft.colors.WHITE), bgcolor=ft.colors.RED),
            ft.Container(
                padding=20,
                content=ft.Column([
                    ft.Text("糟糕！应用初始化遇到错误：", weight=ft.FontWeight.BOLD, size=18),
                    ft.Container(
                        padding=10,
                        bgcolor=ft.colors.GREY_200,
                        border_radius=5,
                        content=ft.Text(error_details, size=12, selectable=True, color=ft.colors.BLACK)
                    )
